...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($emailid_err) && empty($password_err)){
        ...
        $sql = "SELECT id,emailid,password,name,age,occupation,mobilenumber1,mobilenumber2,houseno,street,city,state,pincode,hos_id,doc_id,regdate, slotdate, slotno FROM reguser WHERE emailid = ?";
        ...
        if($stmt = $mysqli->prepare($sql)){
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){                    
                    ...
                    $stmt->bind_result($id, $emailid,$hashed_password,$name,$age,$occupation,$mobilenumber1,$mobilenumber2,$houseno,$street,$city,$state,$pincode,$hos_id,$doc_id, $regdate, $slotdate, $slotno);
                    if($stmt->fetch()){
                        if($password=== $hashed_password){
                           ...
                           // session_start();
                            ...
                            $sql = "SELECT vacno, vacid, vacdate FROM vacuser WHERE id = '$id' AND vacno = (select max(vacno) from vacuser WHERE id = '$id')";
                            ...
                            if($hos_id == NULL){
                                //echo 1;
                                header("location: DisplayHospitals.php");
                            }
                            else if($dif >= 5 && $stm->num_rows == 0){
                               // echo "ENTERED HERE";
  ...
  color: white;
    ...
    <div class="container">
       <form  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype='multipart/form-data'>
           ...
           <h3 style="text-align:center;">LOGIN As User</h3>
        <p style="text-align:center; font-size:16px;">Do not have account.Register <b><a href="register.php">here.</a></b></p>
           <div class="form-group" >
            ...
            <input type="text" class = "form-control" id="emailid" name="emailid" placeholder="Your Registered Mail id here" required><br>
           </div>
           <div class="form-group">
            <label for="password">PASSWORD:</label><?php echo (!empty($password_err)) ? $password_err : ''; ?><br>
            <input type="password" class ="form-control" id="password" name="password" placeholder="Your Password" required><br>
		   ...
		   <a class="foot1" href="ForgotpasswordU.php"><p>Forgot Password?<br></p></a>
           </div>
        </div>
       </form>
    </div>
